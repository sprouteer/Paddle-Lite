...
void PrintDataInt8(std::string name,
                   int8_t* a,
                   const int rows,
                   const int cols) {
  std::cout << "==== " << name << " ====" << std::endl;
  for (int r = 0; r < rows; ++r) {
    for (int c = 0; c < cols; ++c) {
      std::cout << " " << static_cast<int>(a[r * cols + c]);
    }
    std::cout << std::endl;
  }
}
...
TEST(fc, compute) {
  ...
  const int n = 5;
        ...
        auto kernels = KernelRegistry::Global().Create(
            "fc", TARGET(kOpenCL), PRECISION(kInt8), DATALAYOUT(kNCHW));
        ASSERT_FALSE(kernels.empty());
        auto kernel = std::move(kernels.front());
        ...
        auto* x_data = x.mutable_data<int8_t, cl::Buffer>(TARGET(kOpenCL));
        auto* w_data = w.mutable_data<int8_t>();
        auto* bias_data = bias.mutable_data<int8_t>();
        auto* out_data = out.mutable_data<float, cl::Buffer>(TARGET(kOpenCL));

        std::default_random_engine engine;
        std::uniform_real_distribution<int8_t> dist(1, 1);

        std::vector<int8_t> x_source(x_dim.production());
        std::vector<int8_t> w_source(w_dim.production());
        std::vector<int8_t> bias_source(bias_dim.production());

        size_t x_size = x_dim.production() * sizeof(int8_t);
        size_t w_size = w_dim.production() * sizeof(int8_t);
        size_t bias_size = bias_dim.production() * sizeof(int8_t);
        size_t out_size = out_dim.production() * sizeof(int8_t);

        for (size_t i = 0; i < x_dim.production(); ++i) {
          x_source[i] = 1;
          std::cout << "x: " << i << "; " << static_cast<int>(x_source[i])
                    << std::endl;
        }
        for (size_t i = 0; i < w_dim.production(); ++i) {
          w_source[i] = 1;
        ...
        // run cpu ref
        auto* out_ref_data = out_ref.mutable_data<int8_t>(TARGET(kARM));
        gemm_bias<int8_t>(x_source.data(),
                          m,
                          k,
                          w_source.data(),
                          k,
...
#define PRINT_RESULT
#ifdef PRINT_RESULT
        PrintDataInt8("x", static_cast<int8_t*>(x_source.data()), m, k);
        PrintDataInt8("w", static_cast<int8_t*>(w_source.data()), k, n);
        PrintDataInt8("bias", static_cast<int8_t*>(bias_source.data()), 1, n);
        PrintDataInt8("out_ref_data", static_cast<int8_t*>(out_ref_data), m, n);
        PrintData(
            "gpu_out", static_cast<float*>(out_data_from_gpu.data()), m, n);
#endif
...
USE_LITE_KERNEL(fc, kOpenCL, kInt8, kNCHW, def);
